...
architecture: ## serve the interactive C4 architecture model (Structurizr) at http://localhost:6080
	...
	docker run -it --rm --name chap-structurizr $(DOCKER_AS_HOST_USER) -p 6080:8080 -v "$(CURDIR)/architecture:/usr/local/structurizr" structurizr/structurizr:2026.06.28 local

# --- Architecture: validate ---
architecture-validate: ## validate the architecture model DSL (architecture/workspace.dsl)
	docker run --rm $(DOCKER_AS_HOST_USER) -v "$(CURDIR)/architecture:/work" -w /work structurizr/structurizr:2026.06.28 validate -workspace workspace.dsl
...
architecture-export: ## export all architecture diagrams to architecture/diagrams as PNG (needs port 6080 free; also pre-warms viewer thumbnails)
	...
	docker run -d --name chap-structurizr-export $(DOCKER_AS_HOST_USER) -p 6080:8080 -v "$(CURDIR)/architecture:/usr/local/structurizr" structurizr/structurizr:2026.06.28 local >/dev/null; \
	...
	docker run --rm --network container:chap-structurizr-export $(DOCKER_AS_HOST_USER) \
		...
		-v "$(CURDIR)/architecture:/work" -w /work mcr.microsoft.com/playwright:v1.63.0-noble \
		sh -c 'npm i playwright@1.63.0 --no-save --no-fund --no-audit --silent 2>/dev/null && node export-diagrams.js'; \
...
architecture-export-mermaid: ## export the model to Mermaid PNGs under architecture/diagrams/mermaid (alt renderer)
	...
	docker run --rm $(DOCKER_AS_HOST_USER) -v "$(CURDIR)/architecture:/work" -w /work structurizr/structurizr:2026.06.28 export -workspace workspace.dsl -format mermaid -output exports/mermaid >/dev/null; \
...
architecture-check-docs: ## fail if docs/contributor/architecture_model.md is stale vs workspace.dsl
	@set -e; \
	page=docs/contributor/architecture_model.md; \
	before=$$(mktemp); \
	cp "$$page" "$$before"; \
	$(MAKE) --no-print-directory architecture-export-docs >/dev/null; \
	if ! diff -u "$$before" "$$page"; then \
		echo; \
		echo "ERROR: $$page is out of sync with architecture/workspace.dsl."; \
		echo "It has been regenerated in place - review and commit it."; \
		rm -f "$$before"; \
		exit 1; \
	fi; \
	rm -f "$$before"; \
	echo "$$page is in sync with workspace.dsl"
...
architecture-export-plantuml: ## export the model to C4-PlantUML PNGs under architecture/diagrams/plantuml (alt renderer)
	...
	docker run --rm $(DOCKER_AS_HOST_USER) -v "$(CURDIR)/architecture:/work" -w /work structurizr/structurizr:2026.06.28 export -workspace workspace.dsl -format plantuml/c4plantuml -output exports/plantuml >/dev/null; \
