...
1. An entry point in the MLproject file called `train` with parameters `train_data` and `model`
2. An entry point in the MLproject file called `predict` with parameters `historic_data`, `future_data`, `model` and `out_file`
