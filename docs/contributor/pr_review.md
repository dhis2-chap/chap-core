...
PR review looks at one change at a time. Once a week, in the night before
Friday, the `claude-weekly-review` workflow runs a deeper review of the whole
repository with Claude Fable and posts the result as a GitHub issue labelled
`weekly-review`. The report concentrates on code merged during the week, then
re-checks last week's findings, then looks at the rest of `chap_core/` for
accumulated problems such as duplicated logic, dead code, missing tests on
critical paths and drift from the documented architecture.

Each report lists new findings, findings that persist from the previous
report, findings resolved since then, and at most five suggested next actions.
Go through it in the Friday meeting or whenever convenient, turn the actions
worth doing into Jira issues, and close the GitHub issue. The report is
advisory and nothing is blocked by it. It can be run by hand from the Actions
tab, and the model can be changed in the workflow file if the weekly run uses
too much of the plan's quota.

### Deliberate choices the review should not flag

Design decisions that look like problems out of context. The weekly review
reads this list and skips them; add to it when a report keeps flagging
something the team has decided to keep.

- `chap_core/__init__.py` re-exports symbols lazily through PEP 562
  `__getattr__` to keep CLI startup fast. This is intended, not an incomplete
  package interface.
- `chap_core/cli_endpoints/generated_plot_ids.py` is generated by
  `make regen-plot-help` and locked in by a test. It is meant to be committed.
- Model templates run as external processes through MLproject files and
  Docker or Pixi environments. The subprocess handling this needs is by
  design.
