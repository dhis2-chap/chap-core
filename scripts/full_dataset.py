...
from chap_core.rest_api.worker_functions import initialize_gee_client
...
def get_country_data(country_name, old_data=None):
    ...
    periods = [pd.Period(f"{year}-{month}", "M") for year, month in zip(entries["year"], entries["month"])]
...
def join_climate_and_health_data():
    ...
    for country in countries:
        ...
        health_data = health_data.rename(columns={"dengue_cases": "disease_cases", "population_worldpop": "population"})
        ...
        data = {
            location: FullData(
                time_period=st_data[location].time_period,
