...
def _new_transition(P, logits, states):
    ...
    full_mosquito_diffs = [new_eggs-mosquito_diffs[0]]+[mosquito_diffs[i-1]-mosquito_diffs[i] for i in range(1, 5)]+[mosquito_diffs[4]]
    new_mosquito_state = [a + b for a, b in zip(mosquito_state_after_deaths, full_mosquito_diffs)]
    new_state = jnp.array([new_state[..., 0], new_state[..., 1], new_state[..., 2], new_state[..., 3]] + new_mosquito_state).T
