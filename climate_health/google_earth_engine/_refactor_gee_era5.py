...
class GoogleEarthEngine(BaseModel):
    ...
    def fetch_historical_era5_from_gee(self, zip_file_path: str, periodes: Iterable[Periode]) -> SpatioTemporalDict[
        ClimateData]:
    ...
    SpatioTemporalDict[ClimateData]:
        ...
        periodeList = ee.List(
            [ee.Dictionary({"period": p.id, "start_date": p.start_timestamp.date, "end_date": p.end_timestamp.date}) for
             p in periodes])
        ...
        def getPeriode(p, band: Band) -> ee.Image:
            ...
            end = ee.Date(p.get("end_date")).advance(-1,
                                                     "day")  # remove one day, since the end date is inclusive on current format?
            ...
            return getattr(filtered, band.periodeReducer)().set("system:period", p.get("period")).set(
                "system:time_start", start.millis()).set("system:time_end", end.millis())
