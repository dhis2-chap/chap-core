...
from climate_health.datatypes import SummaryStatistics, HealthData, Samples
from climate_health.runners.docker_runner import DockerRunner
from climate_health.runners.runner import TrainPredictRunner
...
class DockerTrainPredictRunner(TrainPredictRunner):
    ...
    def predict(self, model_file_name, historic_data, future_data, output_file):
        command = self._predict_command.format(historic_data=historic_data, future_data=future_data,
                                               model=model_file_name, out_file=output_file)
        return self._docker_runner.run_command(command)
