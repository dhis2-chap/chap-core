...
class RegressionModel:
    ...
    def train(self, st_data: DataSet):
        ...
        seasons = [
            np.array([period.month for period in data.time_period])
            for data in data_list
        ]

        def logpdf(params):
            prior_pdf = [
                jnp.sum(self._priors[name](params[name])) for name in self._priors
            ]
            for i, (data, season) in enumerate(zip(data_list, seasons)):
                rate = self._get_rate(
                    params, data[1:], season[1:], data.disease_cases[:-1], i
                )  # temp_part + lag_part + season_part
                obs_pdf = stats.poisson.logpmf(data.disease_cases[1:], rate).sum()
            return sum(prior_pdf) + obs_pdf

        logpdf(self._initial_params)

        self._param_samples = sample(
            logpdf,
            PRNGKey(348),
        ...
        self._state = {
            location: data.data().disease_cases[-1:]
            for location, data in st_data.items()
        }
    ...
    def predict(self, st_data: DataSet[ClimateData]):
        ...
        predictions = []
        for i, (location, data, season) in enumerate(
            zip(location_names, data_list, seasons)
        ):
        ...
        return DataSet(
            {
                location: prediction
                for location, prediction in zip(st_data.locations(), predictions)
            }
        )
