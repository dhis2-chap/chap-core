...
class SimpleSampler:
    ...
    def train(
        self, time_series: ClimateHealthTimeSeries, init_values=None
    ) -> HealthData:
        T = len(time_series) + 1
        if self._n_states is None:
            init_diffs = np.random.normal(0, 1, (T - 1))
        else:
            init_diffs = np.random.normal(
                logit(0.05), 1, (T - 1, self._n_states)
            ).reshape((T - 1, self._n_states))
        init_params = {
            param_name: np.random.normal(0, 10) for param_name in self._param_names
        }
        ...
        self._param_samples = nuts_sample(
            lp, key, init_dict, self._n_samples, self._n_warmup_samples
        )
        last_params = {
            param_name: self._param_samples[param_name][-1]
            for param_name in self._param_samples
        }
    ...
    def sample(self, climate_data: ClimateData) -> HealthData:
        ...
        params = {
            param_name: self._param_samples[param_name][param_nr]
