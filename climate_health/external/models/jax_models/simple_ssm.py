...
class NaiveSSM:
    ...
    def __init__(self):
        ...
        self._initial_params = {
            "logit_infected_decay": jax.scipy.special.logit(0.9),
            "alpha": 0.1,
        }
    ...
    def _get_priors(self, param_names):
        return {
            name: partial(stats.norm.logpdf, loc=0.1, scale=10) for name in param_names
        }

    def _log_infected_dist(self, params, prev_log_infected, mean_temp, scale=1.0):
        mu = prev_log_infected * jax.scipy.special.expit(
            params["logit_infected_decay"]
        ) + self._temperature_effect(mean_temp, params)
        return partial(
            stats.norm.logpdf, loc=mu, scale=scale
        ), lambda key, shape: jax.random.normal(key, shape) * scale + mu
    ...
    def train(self, data: DataSet[ClimateHealthTimeSeries]):
        ...
        init_params["log_infected"] = {
            location: jnp.log(data.disease_cases + 1) for location, data in data.items()
        }
        init_params["log_observation_rate"] = {
            location: jnp.log(0.1) for location in data.keys()
        }
        prior_pdf_func = lambda params: sum(
            [prior(params[name]) for name, prior in self._prior.items()]
        )

        def obs_pdf_func(params, local_data, location):
            rate = jnp.exp(
                params["log_infected"][location]
                + params["log_observation_rate"][location]
            )
            obs_p = stats.poisson.logpmf(local_data.disease_cases, rate).sum()
            return obs_p

        def infected_pdf_func(params, local_data, location):
            infected_dist, _ = self._log_infected_dist(
                params,
        ...
        if jnp.isnan(first_pdf):
            ...
            logger.error(
                [
                    obs_pdf_func(init_params, local_data, location)
                    for location, local_data in data.items()
                ]
            )
            logger.error(
                [
                    infected_pdf_func(init_params, local_data, location)
                    for location, local_data in data.items()
                ]
            )
        ...
        self._sampled_params = sample(
            logpdf, PRNGKey(0), init_params, self.n_samples, self.n_warmup
        )
        self._last_temperature = {
            location: data.data().mean_temperature[-1]
    ...
    def predict(self, data: DataSet[ClimateData]):
        ...
        for location, local_data in data.items():
            ...
            rate = self._get_rate(
                last_params, location, self._last_temperature[location]
            )
...
class SSMWithSigmoidEffect(SSM):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._prior.update(
            {name: partial(stats.norm.logpdf, loc=0.1, scale=1) for name in param_names}
        )

    def _temperature_effect(self, mean_temp, params):
        return (
            jax.scipy.special.expit(params["beta_temp"] * mean_temp + params["a_temp"])
            * params["b_temp"]
            + params["c_temp"]
        )
...
class SSMWithScaleParam(SSMWithSigmoidEffect):
    ...
    def _log_infected_dist(self, params, prev_log_infected, mean_temp, scale=1.0):
        return super()._log_infected_dist(
            params, prev_log_infected, mean_temp, scale=jnp.exp(params["log_scale"])
        )
