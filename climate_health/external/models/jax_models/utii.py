...
state_or_param = lambda f: tree_util.register_pytree_node_class(
    dataclasses.dataclass(f, frozen=True)
)
...
def index_tree(tree, index):
    if isinstance(tree, jax.Array):
        ...
        return tree.__class__.tree_unflatten(
            None, (index_tree(value, index) for value in tree.tree_flatten()[0])
        )
...
def tree_sample(tree, key, shape=()):
    if isinstance(tree, dict):
        ...
        return {
            name: tree_sample(obj, key, shape=shape)
            for key, (name, obj) in zip(keys, tree.items())
        }
    else:
...
class PydanticTree:
    ...
    def sample(self, key, shape=()):
        ...
        return self.__class__(
            **{
                name: obj.sample(key, shape=shape) if hasattr(obj, "sample") else obj
                for key, (name, obj) in zip(jax.random.split(key, len(d)), d.items())
            }
        )

    def log_prob(self, value: "PydanticTree"):
        return sum(
            getattr(self, field.name).log_prob(getattr(value, field.name))
...
def get_state_transform(params):
    if isinstance(params, tuple):
        ...
        return (
            tuple(T),
            lambda x: tuple(f_i(x_i) for f_i, x_i in zip(f, x)),
            lambda x: tuple(inv_f_i(x_i) for inv_f_i, x_i in zip(inv_f, x)),
        )
    ...
    for field in dataclasses.fields(params):
        if field.type == Positive:
            ...
            default = Normal(np.log(field.default), 1.0)
        elif field.type == np.ndarray:
            converters.append(identity)
            inv_converters.append(identity)
            default = Normal(np.array(field.default), 10.0, ndim=1)
            ...
            converters.append(identity)
            inv_converters.append(identity)
            default = Normal(field.default, 10.0)
        new_fields.append((field.name, float, default))
    new_class = dataclasses.make_dataclass(
        "T_" + params.__name__, new_fields, bases=(PydanticTree,), frozen=True
    )
    tree_util.register_pytree_node_class(new_class)

    def f(transformed: new_class) -> params:
        return params.tree_unflatten(
            None,
            tuple(
                converter(val)
                for converter, val in zip(converters, transformed.tree_flatten()[0])
            ),
        )
    ...
    def inv_f(params: params) -> new_class:
        inv_flat = tuple(
            inv_converter(val)
            for inv_converter, val in zip(inv_converters, params.tree_flatten()[0])
        )
