...
from climate_health.external.models.jax_models.prototype_hierarchical import (
    GlobalSeasonalParams,
    DistrictParams,
    join_global_and_district,
    hierarchical,
    HierarchyLogProbFunc,
    HiearchicalLogProbFuncWithStates,
    HiearchicalLogProbFuncWithDistrictStates,
)
from climate_health.external.models.jax_models.utii import (
    get_state_transform,
    state_or_param,
    index_tree,
    PydanticTree,
...
from .model_spec import PoissonSkipNaN, Normal
...
def get_state_dist_from_params(state_params, n_periods):
    ...
    return MarkovChain(
        lambda state: Normal(state, sigma),
...
def get_state_regression_dist_from_params(state_params, n_periods):
    ...
    def mean_func(mean):
        p = expit(mean)
        return logit(
            p + p * (1 - p) * state_params.beta - p * state_params.gamma + 1e-6
        )

    return MarkovChain(
        lambda state: Normal(mean_func(state), sigma),
...
class SeasonalDistrictParams(DistrictParams):
    alpha: float = 0.0
    beta: float = 0.0
    month_effect: np.ndarray = tuple((0.0,)) * 12
...
class HierarchicalModel:
    ...
    def _set_model(self, data_dict: DataSet[SeasonalClimateHealthData]):
        ...
        class ParamClass(GlobalSeasonalParams):
            ...
            year_effect: np.ndarray = tuple((0.0,)) * n_years

        self._param_class = ParamClass

        def ch_regression(
            params: "ParamClass", given: SeasonalClimateHealthData
        ) -> HealthData:
            log_rate = (
                params.alpha
            ...
            final_rate = (
                jnp.exp(log_rate) * given.population * params.observation_rate + 0.1
            )
    ...
    def train(self, data: DataSet[FullData]):
        ...
        data_dict = {
            key: create_seasonal_data(value.data()) for key, value in data.items()
        }
        ...
        init_params = (
            T_Param().sample(random_key),
            {location: T_ParamD().sample(random_key) for location in data_dict.keys()},
        )
        ...
        assert not jnp.isnan(jax.grad(logprob_func)(last_params)[0].alpha), jax.grad(
            logprob_func
        )(last_params)

    def save_params(self, raw_samples, transform, transformD):
        self.params = (
            transform(raw_samples[0]),
    ...
    def sample(self, data: DataSet[ClimateData], n=1) -> DataSet[HealthData]:
        ...
        true_params = {
            name: join_global_and_district(params[0], params[1][name])
            for name in data_dict.keys()
        }
        return DataSet(
            {
                key: self._regression_model(true_params[key], data_dict[key]).sample(
                    random_key
                )
    ...
    def prediction_summary(
        self, future_weather: DataSet[ClimateData], n_samples=1000
    ) -> DataSet[SummaryStatistics]:
        time_delta = next(iter(future_weather.data())).data().time_period.delta
        future_weather = DataSet(
            {key: value.data()[:1] for key, value in future_weather.items()}
        )
        return self.forecast(
            future_weather, n_samples=n_samples, forecast_delta=1 * time_delta
        )

    def forecast(
        self,
        future_weather: DataSet[ClimateData],
        n_samples=1000,
        forecast_delta=6 * delta_month,
    ) -> DataSet[SummaryStatistics]:
        time_period = next(iter(future_weather.data())).data().time_period
        n_periods = forecast_delta // time_period.delta
        future_weather = DataSet(
            {key: value.data()[:n_periods] for key, value in future_weather.items()}
        )
        time_period = next(iter(future_weather.data())).data().time_period
        ...
        data_dict = {
            key: create_seasonal_data(value.data())
            for key, value in future_weather.items()
        }
        for param_idx, random_key in zip(
            param_idxs, jax.random.split(self._key, num_samples)
        ):
            ...
            for key, value in data_dict.items():
                ...
                samples[key].append(
                    self._sample_from_model(key, new_key, params, true_params, value)
                )
        return DataSet(
            {
                key: get_summary(time_period, np.array(value))
                for key, value in samples.items()
            }
        )

    def _sample_from_model(self, key, new_key, params, true_params, value):
        return self._regression_model(true_params[key], value, *params[2:]).sample(
            new_key
        )
...
class GlobalParams(GlobalSeasonalParams):
    observation_rate: Positive = 0.01
    state_params: StateParams = StateParams(0.0)  # , sigma)
    district_state_params: StateParams = StateParams(0.0)


@state_or_param
class GlobalParams2(GlobalSeasonalParams):
    observation_rate: Positive = 0.01
    state_params: RegressionStateParams = RegressionStateParams(
        0.0, 0.1, 0.1
    )  # , sigma)
    district_state_params: RegressionStateParams = RegressionStateParams(
        0.0, 0.1, 0.1
    )  # , sigma)
...
class HierarchicalStateModel(HierarchicalModel):
    ...
    def _adapt_params(self, params_tuple, data_dict):
        ...
        new_markov_chain = self._state_dist_func(
            prediction_params, len(np.arange(self._idx_range[1] + 1, max_idx + 1))
        )
        key, self._key = jax.random.split(self._key)
        return (
            params,
            district_params,
            np.concatenate([states, new_markov_chain.sample(key)]),
        )
    ...
    def _set_model(self, data_dict: DataSet[SeasonalClimateHealthDataState]):
        ...
        def ch_regression(
            params: "ParamClass", given: SeasonalClimateHealthData, state
        ) -> HealthData:
            idx = given.time_index - min_idx
            log_rate = params.alpha + params.month_effect[given.month - 1] + state[idx]
            final_rate = (
                expit(log_rate) * given.population * params.observation_rate + 0.1
            )
            return PoissonSkipNaN(final_rate)
...
class HierarchicalStateModelD(HierarchicalStateModel):
    ...
    def _adapt_params(self, params_tuple, data_dict):
        ...
        for name, states in states_dict.items():
            prediction_params = dataclasses.replace(
                params.district_state_params, init=states[-1]
            )
            new_markov_chain = self._state_dist_func(
                prediction_params, len(np.arange(self._idx_range[1] + 1, max_idx + 1))
            )
        ...
        prediction_params = dataclasses.replace(
            params.state_params, init=global_states[-1]
        )
        new_markov_chain = self._state_dist_func(
            prediction_params, len(np.arange(self._idx_range[1] + 1, max_idx + 1))
        )
        key, self._key = jax.random.split(self._key)
        global_new_states = np.concatenate(
            [global_states, new_markov_chain.sample(key)]
        )
        return params, district_params, (global_new_states, d)

    def _sample_from_model(self, key, new_key, params, true_params, value):
        return self._regression_model(
            true_params[key], value, params[2][1][key] + params[2][0]
        ).sample(new_key)

    def _add_init_params(self, init_params):
        init_states = {
            name: np.zeros(self._idx_range[1] + 1 - self._idx_range[0])
...
class _SimpleMarkovChain:
    def __init__(self, state_params):
        self._chain = MarkovChain(
            lambda state: Normal(state, state_params.sigma),
