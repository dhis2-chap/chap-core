...
class NegativeBinomial2:
    ...
    def log_prob(self, x: Any) -> Any:
        ...
        second_term = x * (
            jnp.log(self.mu) + jnp.log(self.alpha) - jnp.log(1 + self.alpha * self.mu)
        )
...
class LogNormal:
    ...
    def log_prob(self, value: float):
        ...
        return (
            ...
            - np.log(self.sigma)
            - 0.5 * np.log(2 * np.pi)
            + np.log(value - self.mu) ** 2 / (2 * self.sigma**2)
        )
...
class IsSSMSpec(Protocol):
    ...
    def observation_distribution(
        self, params: dict[str, Any], predictors: dict[str, Any] = None
    ) -> IsDistribution: ...
...
class DictDist:
    ...
    def log_prob(self, x):
        return sum(
            self.dist_dict[name].log_prob(x[name]) for name in self.dist_dict.keys()
        )
...
class IsSSMForecaster(Protocol):
    def __init__(
        self, model_spec: IsSSMSpec, training_params: dict[str, Any] = None
    ): ...
    ...
    def forecast(
        self, data: dict[str, Any], n: int, forecast_delta: int
    ) -> dict[str, Any]: ...
...
class SSMForecasterNuts:
    ...
    def save(self, filename: str):
        ...
        with open(filename, "wb") as f:
            pickle.dump(
                (self._sampled_params, self._last_predictors, self._model_spec), f
            )
    ...
    def _get_local_params(self, params, location):
        ...
        locals = {
            param_name: params[param_name][location]
            for param_name in self._model_spec.location_params
        }
    ...
    def train(self, data: DataSet[ClimateHealthTimeSeries]):
        orig_data = data
        logger.info(
            f"Training model {self._model_spec} with {len(data.locations())} locations"
        )
        data = {name: remove_nans(data.data()) for name, data in data.items()}
        ...
        def obs_pdf_func(states, local_params, local_data):
            return (
                self._model_spec.observation_distribution(
                    states,
                    local_params,
                    {
                        name: getattr(local_data, name)
                        for name in self._model_spec.predictors
                    },
                )
                .log_prob(local_data.disease_cases)
                .sum()
            )

        def infected_pdf_func(states, params, local_data):
            previous_state = {
                param_name: states[param_name][:-1]
            ...
            dist = self._model_spec.state_distribution(
                previous_state,
                params,
                {
                    name: getattr(local_data, name)[:-1]
                    for name in self._model_spec.predictors
                },
            )
        ...
        if jnp.isnan(first_pdf):
            ...
            logger.error(
                [
                    obs_pdf_func(
                        self._get_state_params(init_params, location),
        ...
        self._sampled_params = sample(
            logpdf,
            PRNGKey(0),
            init_params,
            self._training_params.n_samples,
            self._training_params.n_warmup,
        )
        self._last_predictors = {
            location: {
                name: getattr(data.data(), name)[-1]
                for name in self._model_spec.predictors
            }
    ...
    def predict(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for location, local_data in data.items():
            ...
            new_state = self._model_spec.state_distribution(
                index_tree(self._get_state_params(last_params, location), -1),
                location_params,
                self._last_predictors[location],
            )["logit_infected"].mu
            rate = self._model_spec.observation_distribution(
                {"logit_infected": new_state}, location_params
            ).rate
    ...
    def prediction_summary(
        self, data: DataSet = None, num_samples: int = 100
        ...
        for i, key in zip(param_idxs, jax.random.split(sample_key, num_samples)):
            ...
            for location in locations:
                ...
                new_state = self._model_spec.state_distribution(
                    last_state, location_params, self._last_predictors[location]
                ).sample(state_key)
                samples[location].append(
                    self._model_spec.observation_distribution(
                        new_state, location_params, self._last_predictors[location]
                    ).sample(obs_key)
                )
        # time_period = next(iter(data.data())).data().time_period[:1]
    ...
    def forecast(
        ...
        for i, key in zip(param_idxs, jax.random.split(sample_key, num_samples)):
            param = extract_sample(i, self._sampled_params)
            for location, local_data in data.items():
                rates = self._get_rates_forwards(
                    param, location, local_data.data(), n_periods
                )
    ...
    def _get_init_params(self, data):
        ...
        for name in self._model_spec.state_params:
            init_params[name] = {
                location: jnp.full(len(data), 0.1) for location, data in data.items()
            }
    ...
    def _get_prior(self):
        log_pdf = lambda params: sum(
            Normal(0, 10).log_prob(params[name])
            for name in self._model_spec.global_params
        ) + sum(
            Normal(0, 10).log_prob(value)
            for name in self._model_spec.location_params
