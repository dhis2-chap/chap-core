...
class PopulationPoisson:
    ...
    def _dist(self):
        return PoissonSkipNaN(
            self.state.infected_proportion
            * self.params.observation_rate
...
class InfectedDistribution:
    ...
    def _dist(self):
        return Normal(
            self.state.infected_proportion * self.params.infected_decay,
            self.params.state_sigma,
        )
...
def state_distribution(
    previous_raw_state: StateSpace[StateParams],
    params: GlobalParams,
    predictors: Any = None,
...
def observation_distribution(
    state: StateParams, global_params: GlobalParams, predictors: dict[str, Any]
) -> IsDistribution:
    return PoissonSkipNaN(
        state.infected * global_params.observation_rate * predictors["population"]
    )
