...
from .api_types import FeatureCollectionModel as DFeatureCollectionModel, FeatureModel as DFeatureModel
...
country_names = ['brazil', 'mexico', 'el salvador', 'paraguay', 'peru', 'colombia', 'ecuador', 'nicaragua', 'panama',
                 'argentina', 'indonesia', 'philippines', 'thailand', 'vietnam', 'laos', 'malaysia', 'cambodia',
                 'singapore']
country_codes_l = ['BRA', 'MEX', 'SLV', 'PRY', 'PER', 'COL', 'ECU', 'NIC', 'PAN', 'ARG', 'IDN', 'PHL', 'THA', 'VNM',
                   'LAO', 'MYS', 'KHM', 'SGP']
...
def get_area_polygons(country: str, regions: list[str]) -> FeatureCollectionModel:
    ...
    return DFeatureCollectionModel(
        ...
        features=[add_id(feature_dict[normalize_name(region)]) for region in regions if
                  normalize_name(region) in feature_dict]
    )
