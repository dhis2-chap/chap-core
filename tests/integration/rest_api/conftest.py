...
def predictions(future_periods, org_units):
    return [
        PredictionSamplesEntry(
            period=tp,
            org_unit=ou,
            values=[float(tp_id + 0.1 * s) for s in range(10)]
        )
        for tp_id, tp in enumerate(future_periods) for ou in org_units
    ]
...
def forecasts(seen_periods, org_units, backtest_params):
    ...
    for start in range(start_split, start_split + backtest_params.n_splits):
        forecasts.extend([
            ...
            for t in range(backtest_params.n_periods) for ou in org_units])
