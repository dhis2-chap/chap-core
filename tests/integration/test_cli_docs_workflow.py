...
# Use a lightweight GitHub model for testing (eval requires path or URL, not built-in names)
TEST_MODEL = "https://github.com/dhis2-chap/minimalist_example_lag"


@pytest.mark.slow
def test_eval_with_github_model():
    """Test eval command with GitHub model and example data."""
...
def test_plot_backtest():
    """Test plot-backtest command with eval output."""
    if not EXAMPLE_DATA.exists():
        pytest.skip(f"Example data not found: {EXAMPLE_DATA}")
...
def test_export_metrics():
    """Test export-metrics command with eval output."""
    if not EXAMPLE_DATA.exists():
        pytest.skip(f"Example data not found: {EXAMPLE_DATA}")
