...
from climate_health.external.models.jax_models.deterministic_seir_model import SIRParams, Params, SIRState, MarkovChain, \
    get_markov_chain, main_sir, SIRObserved, get_state_transform, transformed_diff_distribution, \
...
def test_probabilistic_sir():
    ...
    init_params = T_Param().sample(jax.random.PRNGKey(0))
    init_states = main_sir(transform(init_params), observations)[1].sample(PRNGKey(0))
    params = (init_params, f(init_states))
    print(jax.value_and_grad(logprob_func)(params))
    #
    raw_samples, _ = sample(logprob_func, PRNGKey(0), params, num_samples=100, num_warmup=100)
    samples = transform(raw_samples)
