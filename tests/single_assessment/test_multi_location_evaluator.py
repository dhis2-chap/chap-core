...
def r_model_predictions():
    ...
    T = len(time_period)
    d = {'oslo': SummaryStatistics(time_period, mean=[0.5]*T, std=[0.5]*T, median=[0.5]*T, min=[0.5]*T, max=[0.5]*T, quantile_low=[0.5]*T, quantile_high=[0.5]*T),
         'bergen': SummaryStatistics(time_period, mean=[0.5]*T, std=[0.5]*T, median=[0.5]*T, min=[0.5]*T, max=[0.5]*T, quantile_low=[0.5]*T, quantile_high=[0.5]*T)}
    return DataSet(d)
...
def test_multi_location_evaluator_r_model(full_data, good_predictions, r_model_predictions):
    evaluator = MultiLocationEvaluator(model_names=['good_model', 'r_model'],
                                       truth=full_data)
    evaluator.add_predictions('good_model', good_predictions)
