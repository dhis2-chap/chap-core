...
from climate_health.dhis2_interface.json_parsing import json_to_pandas, parse_disease_data
...
def test_json_to_pandas(json_data):
    df = json_to_pandas(json_data, name_mapping={'time_period': 2, 'disease_cases': 3, 'location': 1})
    print(all(df.time_period == ['2024-02', '2024-03']))


def test_parse_disease_data(json_data):
    data = parse_disease_data(json_data, name_mapping={'time_period': 2, 'disease_cases': 3, 'location': 1})
