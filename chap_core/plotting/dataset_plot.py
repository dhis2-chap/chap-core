...
class DatasetPlot(ABC):
    ...
    def _get_colnames(self) -> filter:
        colnames = filter(
            ...
            self._df.columns)
        colnames = filter(lambda name: self._df[name].dtype.name in ('float64', 'int64', 'bool', 'int32', 'float32'),
                          colnames)
...
def test_standardized_feature_plot(df: pd.DataFrame):
    df['ideal_temperature'] = (df['mean_temperature'] > 25) & (
                df['mean_temperature'] <= 30)  # Assuming mean_temperature is the predictor
