...
from chap_core.assessment.flat_representations import convert_backtest_to_flat_forecasts, \
...
class BackTestPlot:
    ...
    def _error_band(self, q_low, q_high):
        error_band = alt.Chart(self._forecast).mark_errorband().encode(
            x='time_period:T',
    ...
    def plot(self) -> FacetChart:
        line = alt.Chart(self._forecast).mark_line().encode(
            x='time_period:T',
            y='q_50:Q',
        )
        ...
        observations = alt.Chart(self._observed).mark_line(color='black').encode(
            x='time_period:T',
            y='disease_cases:Q',
        )
        full_plot =  line + error1 + error2 + observations
        return full_plot.facet(
