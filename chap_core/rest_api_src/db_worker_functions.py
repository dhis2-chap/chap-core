...
from chap_core.rest_api_src.worker_functions import (
    harmonize_health_dataset,
    WorkerConfig,
)
...
def run_backtest(
    ...
    estimator = registry.get_model(
        info.model_id, ignore_env=info.model_id.startswith("chap_ewars")
    )
...
def run_prediction(
    ...
    estimator = registry.get_model(
        estimator_id, ignore_env=estimator_id.startswith("chap_ewars")
    )
    predictions = forecast_ahead(estimator, dataset, n_periods)
    db_id = session.add_predictions(
        predictions, dataset_id, estimator_id, name, metadata
    )
    assert db_id is not None
    return db_id
...
def harmonize_and_add_health_dataset(
    health_dataset: FullData,
    name: str,
    session: SessionWrapper,
    worker_config=WorkerConfig(),
) -> FullData:
    health_dataset = InMemoryDataSet.from_dict(health_dataset, HealthPopulationData)
    dataset = harmonize_health_dataset(
        health_dataset, usecwd_for_credentials=False, worker_config=worker_config
    )
    db_id = session.add_dataset(
        name, dataset, polygons=health_dataset.polygons.model_dump_json()
    )
    return db_id
