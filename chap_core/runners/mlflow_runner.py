...
class MlFlowTrainPredictRunner(TrainPredictRunner):
    ...
    def predict(self, model_file_name, historic_data, future_data, output_file, polygons_file_name=None):
        ...
        params = {"historic_data": str(historic_data), "future_data": str(future_data),"model": str(model_file_name), "out_file": str(output_file)}
        extra_params = {'model_config': str(
                self.model_configuration_filename) if self.model_configuration_filename else None, }
